export function solve(input: string): number | string {
  ...
  let groups = 0;
  const notFound = new Set(pipes.pipes());

  while (notFound.size) {
    const first = notFound.values().next().value!;
    const connected = pipes.connected(first);

    for (const pipe of connected) {
      notFound.delete(pipe);
    }
